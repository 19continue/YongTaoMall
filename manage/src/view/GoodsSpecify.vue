<template>
    <div style="padding: 30px;">
        ...
        <Drawer ref="editDrawer" :show="show" :attribute="editAttribute" :formData="formData" :labelData="tableColumn"
            :url="url.edit" :rules="rules" @refresh="dataRefresh" @getSelectOptionparent="(row)=>{ getSelectOptionParent('edit',row) }">
        </Drawer>
    </div>
</template>
...
export default {
    ...
    data() {
        ...
        return {
            tableColumn: [
                { type: 'text', label: 'ID', prop: 'class_id', sortable: false, align: 'center', addHide: true, notEditable: true },
